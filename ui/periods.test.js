...
test('buildPlanData: 月の中身が実際に変わる（別の暦月＝別のアーク内容が出る）', async () => {
  const data = await buildPlanData(localStorages());
  // 業務意図: 6つの別々の暦月を出し、月を切り替えるとその月のアーク内容が出る。
  // （フェーズ「文字列」の全一意は年間計画の偶然＝中体連月が同名なので根拠にしない）
  const displayMonths = data.months.map((m) => m.displayMonth);
  assert.equal(new Set(displayMonths).size, 6, '6つの別々の暦月を並べる');
  assert.notEqual(
    data.months[0].month.headline,
    data.months[2].month.headline,
    '別の月へ切り替えるとその月の内容（狙い）が変わる＝実切替の中身差',
  );
});
