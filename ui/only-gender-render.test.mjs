...
// 業務意図: 背景の箱・枠をやめ、数字そのものに白い縁取り（text-shadow ハロー）を付ける。オンリー専用の
// .tk 上書きは持たない（オンリーと2列で時刻が完全に同一の見た目）。
test('(tk-parity) 時刻(.tk)は箱なし・白縁取りの基底スタイルを両モード共通で使う（オンリー専用上書き無し）', async () => {
  const data = await buildPlanData(localStorages());
  ...
  // 基底 .spine-clk .tk（メイン宣言＝text-shadow を含むルール）を取り出す。
  // 同名の別ルール（`.spine-end .spine-clk .tk` の色上書き・メディアクエリの font-size）を拾わないよう、
  // 全 `.spine-clk .tk{…}` ブロックから text-shadow を含むものを選ぶ。
  const tkBlocks = [...css.matchAll(/\.spine-clk \.tk\{([^}]*)\}/g)].map((mm) => mm[1]);
  const decl = tkBlocks.find((d) => /text-shadow/.test(d));
  assert.ok(decl, '基底 .spine-clk .tk（縁取りを持つメイン宣言）が存在する');
  assert.match(decl, /background:none/, '背景の箱を打ち消す（background:none＝線マスク用の箱を廃止）');
  assert.match(decl, /padding:0/, 'pad を打ち消す（箱の余白なし）');
  assert.doesNotMatch(decl, /border\s*:/, '枠線を持たない');
  assert.match(decl, /text-shadow\s*:[^;]*var\(--surface\)/, '数字に白（var(--surface)）の縁取り（text-shadow ハロー）を持つ');
  assert.match(decl, /font-size:13px/, 'font-size は少し大きめ（13px）');
