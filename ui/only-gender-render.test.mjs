...
test('(tk-parity) 時刻(.tk)は18px・text-shadowなし・地色マスクのみの基底スタイルを両モード共通で使う', async () => {
  ...
  // 基底 .spine-clk .tk（メイン宣言＝font-size:18px を含むルール）を取り出す。
  // 同名の別ルール（`.spine-end .spine-clk .tk` の色上書き・メディアクエリの font-size:15px）を拾わないよう、
  // 全 `.spine-clk .tk{…}` ブロックから 18px を含むものを選ぶ。
  const tkBlocks = [...css.matchAll(/\.spine-clk \.tk\{([^}]*)\}/g)].map((mm) => mm[1]);
  const decl = tkBlocks.find((d) => /font-size:18px/.test(d));
  ...
  assert.doesNotMatch(decl, /border-radius/, '角丸を持たない');
  assert.doesNotMatch(decl, /box-shadow/, '影を持たない');
});
