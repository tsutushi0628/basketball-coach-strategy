...
// ── 複数practice展開: 対人ブロックに 1on1/3on3/5on5 が並ぶとき全てが rows に出る ──────

test('複数practice展開: 対人ブロックの中間3on3・末尾5on5が rotation rows の name 集合に現れる', () => {
  // 対人ブロックに practice が3本（1on1→3on3→5on5）。先頭1本だけ膨らませると 3on3/5on5 が
  // 可視 rows から脱落する不具合の回帰テスト。3本それぞれが見える行(rows)に出ることを保証する。
  const pd = pdWithBlocks('火', [
    { block: '対人', isBundle: false, items: [
      { name: 'コースチェックドリル', mode: 'practice', minutes: 10 }, // 1on1相当
      { name: '3on3ディフェンスドリル', mode: 'practice', minutes: 10 }, // 3on3=チームディフェンス
      { name: '5対5（オールコートゲーム）', mode: 'practice', minutes: 15 }, // 5on5
    ]},
  ]);
  const pool = [
    { name: 'FillA', mode: 'self', minutes: 10, category: 'C', video: null, alternatives: [] },
    { name: 'FillB', mode: 'self', minutes: 10, category: 'C', video: null, alternatives: [] },
    { name: 'FillC', mode: 'self', minutes: 10, category: 'C', video: null, alternatives: [] },
  ];
  const rot = buildRotation(pd, pool);

  // coachSide 側（practice セル）の name を全 rotation 行から集める
  const practiceNames = new Set();
  for (const row of rot.rows) {
    if (row.type !== 'rotation') continue;
    const coachCell = row.coachSide === '男子' ? row.boys : row.girls;
    if (coachCell.mode === 'practice') practiceNames.add(coachCell.name);
  }

  assert.ok(practiceNames.has('5対5（オールコートゲーム）'), '5on5 が rotation rows に現れる');
  assert.ok(practiceNames.has('3on3ディフェンスドリル'), '3on3 が rotation rows に現れる');
  assert.ok(practiceNames.has('コースチェックドリル'), '1on1 が rotation rows に現れる');

  // 不変条件（practiceはcoachSideのみ・左右別ドリル）を満たし時計不変も保つ
  assert.deepEqual(findRotationViolations(rot.rows), [], '不変条件違反ゼロ');
  const rowsTotal = rot.rows.reduce((s, r) => s + r.minutes, 0);
...
test('E2E火曜: 実カタログ出力で16:05開始・固定6ブロック・violations=0・主自走together', async () => {
  ...
  const tueMinutes = config.schedule.find((s) => s.day === '火').minutes; // SCHEDULE から窓を引く
  ...
  let cur = START_MIN;
  const BLOCK_LABEL = { アップ: 'アップ', ファンダ: 'ファンダメンタル', シュート: 'シュート', 対人: '対人', ラン: '走り込み', 静的: '静的ストレッチ' };
  ...
  // 開始 16:05・窓は SCHEDULE の火曜分（120分）・終了は開始＋窓 の検証
  assert.equal(pd.start, '16:05', '開始 16:05');
  assert.equal(pd.totalMinutes, tueMinutes, `窓 ${tueMinutes}分（SCHEDULE）`);
  assert.equal(pd.end, hhmm2(START_MIN + tueMinutes), `終了 ${hhmm2(START_MIN + tueMinutes)}`);
  ...
  // rows 合計 = 窓（火曜の SCHEDULE 分）
  const rowsTotal = rot.rows.reduce((s, r) => s + r.minutes, 0);
  assert.equal(rowsTotal, tueMinutes, `rows合計(${rowsTotal}) === ${tueMinutes}分（窓一致）`);
