...
test('planWeek(catalog): 導入済みを次週入力にすると当該ドリルが平日に自走で出る', async () => {
  ...
  // In the rotation-model weekday view those introduced drills read as 自走
  ...
  for (const dp of week2.weekday_groups) {
    ...
    for (const round of dp.rounds) {
      if (round.kind === 'rotation') {
        assert.notEqual(round.practice.mode, 'lecture', `平日(${dp.day}) rotation.practice にレクチャ表示が残っている: ${round.practice.name}`);
        for (const sf of round.self_fill) {
          assert.notEqual(sf.mode, 'lecture', `平日(${dp.day}) self_fill にレクチャ表示が残っている: ${sf.name}`);
        }
      } else if (round.kind === 'both_self') {
        for (const d of round.drills) {
          assert.notEqual(d.mode, 'lecture', `平日(${dp.day}) both_self にレクチャ表示が残っている: ${d.name}`);
        }
      }
    }
  }
