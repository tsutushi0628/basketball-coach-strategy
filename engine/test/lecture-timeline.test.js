...
test('planWeek(catalog): 導入済みを次週入力にすると当該ドリルが平日に自走で出る', async () => {
  ...
  // 注: 「平日に出た既習レクチャ型を自走として“表示”する」のは描画側（UI plan-data /
  // 旧 groups.js）の責務。エンジンの day ブロックはドリル本来の coaching_mode を保持する
  // のが正しい（土レクチャ host の抽出が同じ mode を見るため）。表示ルールは UI 側で担保。
