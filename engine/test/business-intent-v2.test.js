...
test('V5: ファンダとシュートが別々の固定ブロックとして存在する（旧 技術 一括の分離）', async () => {
  ...
  for (const day of plan.days) {
    ...
    if (funda && shoot) {
      daysWithBoth += 1;
      // ファンダ holds handling/passing/footwork (or 習得 finishing); シュート holds catch&shoot / 反復 finishing.
      for (const it of shoot.items) {
        assert.ok(
          ['シュート', 'フィニッシュ(ゴール下/レイアップ)'].includes(it.category),
          `シュートブロックに場違いカテゴリ「${it.category}」(${it.name})`,
        );
      }
      for (const it of funda.items) {
        assert.ok(
          ['ハンドリング/ドリブル', 'パス&スペーシング', 'フットワーク/アジリティ/ピボット', 'フィニッシュ(ゴール下/レイアップ)'].includes(it.category),
          `ファンダブロックに場違いカテゴリ「${it.category}」(${it.name})`,
        );
      }
    }
  }
...
test('V8: changing the input changes the allocation (responsive, not fixed)', async () => {
  ...
  const below = planWeek(drills, config, inputWith(baseInput, { 'FT率': { latest: 30 } }), weekFocus);
  const met = planWeek(drills, config, inputWith(baseInput, { 'FT率': { latest: 70 } }), weekFocus);
  assert.notDeepEqual(
