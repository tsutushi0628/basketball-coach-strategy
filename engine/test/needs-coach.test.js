...
test('owner例: 人付きレイアップは要コーチ（守備を付けたフィニッシュ）', () => {
  // シュート category alone would be 自走, but a live defender ("人付き"/
...
test('catalog全件: 全ドリルが要コーチ/自走のいずれかに分類され、両クラスが存在する', async () => {
  ...
  assert.equal(drills.length, 217, 'catalog should hold all 217 drills (added defensive footwork FTW-019 ラテラル・シャッフル＆フロアタッチ)');
