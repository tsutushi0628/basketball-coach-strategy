...
test('実践: 守備/ライブ/人付き等のキーワードを持つ要コーチドリルは practice', () => {
  for (const sub of ['ライブ1対1', 'クローズアウト守備', '人付きフィニッシュ', '2on2の合わせ']) {
    assert.equal(
      coachingMode(drill({ category: 'シュート', name: 'レイアップ', sub_skill: sub, mastery_stage: '反復' })),
      'practice',
      `「${sub}」は実践`,
    );
  }
...
test('境界: 新規習得でも対人キーワードがあれば実践（レクチャに落ちない）', () => {
  ...
  assert.equal(
    coachingMode(drill({ category: 'シュート', name: '人付きレイアップ', sub_skill: 'ディフェンスを背負った習得', mastery_stage: '習得' })),
    'practice',
  );
});
...
test('coachingModeReason: 理由は判定と常に一致する', () => {
  const cases = [
    ...
    drill({ category: 'シュート', name: 'レイアップ', sub_skill: 'ライブ', mastery_stage: '反復' }),
...
test('catalog全件: 全ドリルが3分類のいずれかに入り、3クラスとも非空', async () => {
  ...
  assert.equal(drills.length, 217, 'catalog は217件（守備フットワーク FTW-019 ラテラル・シャッフル＆フロアタッチ を追加）');
